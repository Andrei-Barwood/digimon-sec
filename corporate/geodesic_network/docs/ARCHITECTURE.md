# Arquitectura - geodesic_network

## Visión General

geodesic_network es un módulo de ciberseguridad implementado como parte del **Snocomm Security Suite**.
...
La clase `GeodesicNetwork` es el punto de entrada principal. Implementa la lógica de negocio y orquesta las operaciones de seguridad.

**Responsabilidades:**
- `add_connection()`: Añade una conexión al monitor
- `analyze_traffic()`: Analiza el tráfico acumulado
- `analyze()`: Analiza tráfico: usa conexiones existentes o acepta nuevas
- `validate()`: Valida datos de entrada
- `get_info()`: Obtener información del módulo
- `reset()`: Resetea el estado del monitor
...
**Modelos Principales:**
- `ModuleConfig`: Configuration model for geodesic_network
- `NetworkConnection`: Network connection information
- `TrafficAnalysis`: Result of traffic analysis
