...
# HelixTrace requiere la ruta de un archivo (log, txt, etc.) para analizar
result = modulo.analyze(artifact_path="/var/log/syslog")
print(result)
```

## Configuración Avanzada

Puede configurar helix_trace pasando un diccionario al constructor:

```python
modulo = HelixTrace(config={
...
result = modulo.analyze(artifact_path=sample_log)

# 4. Procesamiento de resultados
if result.status == "success":
    print(f"✅ Operación exitosa: {result.message}")
    
    # Acceder a los datos específicos de helix_trace
...
```python
data = "ruta/al/archivo.log" # Datos a validar (HelixTrace valida que el input sea string válido)
