# Arquitectura - fractal_veil

## Visión General

fractal_veil es un módulo de ciberseguridad implementado como parte del **Snocomm Security Suite**.

- **Misión**: Clemens Point
- **Rol de Seguridad**: privacy-auditor
...
La clase `FractalVeil` es el punto de entrada principal. Implementa la lógica de negocio y orquesta las operaciones de seguridad.

**Responsabilidades:**
- `audit_policy()`: Audita política de privacidad de un objetivo
- `analyze()`: Ejecuta análisis: auditoría de privacidad
...
**Modelos Principales:**
- `ModuleConfig`: Configuration model for fractal_veil
- `PrivacyCheck`: Individual privacy policy check result
- `PrivacyAudit`: Privacy policy audit result
