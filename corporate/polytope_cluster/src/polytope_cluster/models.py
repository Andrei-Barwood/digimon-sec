"""
Data models for polytope_cluster (Production).
...
class ModuleConfig(BaseModel):
    """Configuration model for polytope_cluster"""

    model_config = ConfigDict(frozen=True)

    name: str = Field(default="Polytope Cluster", description="Module name")
    strict_mode: bool = Field(default=True, description="Fail on missing critical settings")
    require_rbac: bool = Field(default=True, description="Require RBAC enabled")
    require_psp: bool = Field(default=True, description="Require pod security policies")
    require_etcd_encryption: bool = Field(default=True, description="Require etcd encryption")
