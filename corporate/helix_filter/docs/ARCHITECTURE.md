# Arquitectura - helix_filter

## Visión General

helix_filter es un módulo de ciberseguridad implementado como parte del **Snocomm Security Suite**.

- **Misión**: Good, Honest Snake Oil
- **Rol de Seguridad**: threat-filter
...
La clase `HelixFilter` es el punto de entrada principal. Implementa la lógica de negocio y orquesta las operaciones de seguridad.

**Responsabilidades:**
- `analyze_threats()`: Analiza una lista de IOCs
- `analyze()`: Ejecuta análisis: un IOC o múltiples
...
**Modelos Principales:**
- `ModuleConfig`: Configuration model for helix_filter
- `IOCMatch`: IOC match information
- `ThreatAnalysis`: Result of threat analysis
