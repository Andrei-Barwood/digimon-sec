# Arquitectura - geodesic_directory

## Visión General

geodesic_directory es un módulo de ciberseguridad implementado como parte del **Snocomm Security Suite**.

- **Misión**: American Distillation
- **Rol de Seguridad**: ldap-manager
...
La clase `GeodesicDirectory` es el punto de entrada principal. Implementa la lógica de negocio y orquesta las operaciones de seguridad.

**Responsabilidades:**
- `search_entries()`: Busca entradas LDAP
- `analyze_directory()`: Analiza el directorio LDAP
- `analyze()`: Ejecuta análisis: analizar directorio o buscar entradas
...
**Modelos Principales:**
- `ModuleConfig`: Configuration model for geodesic_directory
- `LDAPEntry`: LDAP directory entry
- `LDAPAnalysis`: Result of LDAP analysis
