"""
Data models for lemniscate_horizon (Production).
...
class ModuleConfig(BaseModel):
    """Configuration model for lemniscate_horizon"""

    model_config = ConfigDict(frozen=True)

    name: str = Field(default="Lemniscate Horizon", description="Module name")
    strict_mode: bool = Field(default=True, description="Fail on missing critical settings")
    min_passed_controls: int = Field(default=1, description="Minimum passed controls")
    require_evidence: bool = Field(default=True, description="Require evidence collection")
    require_continuous_monitoring: bool = Field(default=True, description="Require continuous monitoring")
