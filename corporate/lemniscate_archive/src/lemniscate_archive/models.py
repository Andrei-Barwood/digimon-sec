"""
Data models for lemniscate_archive (Production).
...
class ModuleConfig(BaseModel):
    """Configuration model for lemniscate_archive"""

    model_config = ConfigDict(frozen=True)

    name: str = Field(default="Lemniscate Archive", description="Module name")
    strict_mode: bool = Field(default=True, description="Fail on missing critical settings")
    min_retention_days: int = Field(default=7, description="Minimum backup retention days")
    max_backup_age_hours: int = Field(default=24, description="Maximum age of last backup in hours")
    require_cross_region: bool = Field(default=True, description="Require cross-region replication")
