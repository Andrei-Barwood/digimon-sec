# Arquitectura - helix_biometric

## Visión General

helix_biometric es un módulo de ciberseguridad implementado como parte del **Snocomm Security Suite**.

- **Misión**: My Last Boy
- **Rol de Seguridad**: biometric-handler
...
La clase `HelixBiometric` es el punto de entrada principal. Implementa la lógica de negocio y orquesta las operaciones de seguridad.

**Responsabilidades:**
- `register_biometric()`: Registra un template biométrico
- `analyze_biometrics()`: Analiza todos los templates biométricos
- `analyze()`: Ejecuta análisis: analizar templates o registrar nuevo
...
**Modelos Principales:**
- `ModuleConfig`: Configuration model for helix_biometric
- `BiometricData`: Biometric data information
- `BiometricAnalysis`: Result of biometric analysis
...
```python
config = {
    "supported_types": ["fingerprint", "face", "iris", "voice"],  # Configura supported_types
}
```
