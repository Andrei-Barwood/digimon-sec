"""
Data models for simplex_container (Production).
...
class ModuleConfig(BaseModel):
    """Configuration model for simplex_container"""

    model_config = ConfigDict(frozen=True)

    name: str = Field(default="Simplex Container", description="Module name")
    strict_mode: bool = Field(default=True, description="Fail on missing critical settings")
    require_signed_images: bool = Field(default=True, description="Require signed images")
    require_read_only_fs: bool = Field(default=True, description="Require read-only filesystem")
