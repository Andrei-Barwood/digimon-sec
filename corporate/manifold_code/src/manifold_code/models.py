"""
Data models for manifold_code (Production).
...
class ModuleConfig(BaseModel):
    """Configuration model for manifold_code"""

    model_config = ConfigDict(frozen=True)

    name: str = Field(default="Manifold Code", description="Module name")
    strict_mode: bool = Field(default=True, description="Fail on missing critical settings")
    require_remote_state_encrypted: bool = Field(default=True, description="Require encrypted remote state")
    require_plan_approval: bool = Field(default=True, description="Require plan approval")
    require_modules_pinned: bool = Field(default=True, description="Require pinned module versions")
