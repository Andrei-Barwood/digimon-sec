# Arquitectura - helix_standard

## Visión General

helix_standard es un módulo de ciberseguridad implementado como parte del **Snocomm Security Suite**.

- **Misión**: The Gunslinger
- **Rol de Seguridad**: pci-dss-validator
...
La clase `HelixStandard` es el punto de entrada principal. Implementa la lógica de negocio y orquesta las operaciones de seguridad.

**Responsabilidades:**
- `audit_compliance()`: Audita cumplimiento PCI-DSS de un objetivo
- `analyze()`: Ejecuta análisis: auditoría PCI-DSS
...
**Modelos Principales:**
- `ModuleConfig`: Configuration model for helix_standard
- `PCI_DSSCheck`: Individual PCI-DSS compliance check result
- `PCI_DSSComplianceReport`: PCI-DSS compliance report
