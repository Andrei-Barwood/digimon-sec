# Arquitectura - geodesic_session

## Visión General

geodesic_session es un módulo de ciberseguridad implementado como parte del **Snocomm Security Suite**.

- **Misión**: Polite Society
- **Rol de Seguridad**: session-manager
...
La clase `GeodesicSession` es el punto de entrada principal. Implementa la lógica de negocio y orquesta las operaciones de seguridad.

**Responsabilidades:**
- `create_session()`: Crea una nueva sesión
- `validate_session()`: Valida una sesión
- `analyze_sessions()`: Analiza todas las sesiones activas
- `analyze()`: Ejecuta análisis: analizar sesiones o crear sesión
...
**Modelos Principales:**
- `ModuleConfig`: Configuration model for geodesic_session
- `Session`: Session information
- `SessionAnalysis`: Result of session analysis
