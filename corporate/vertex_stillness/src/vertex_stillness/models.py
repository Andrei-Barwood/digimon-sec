"""
Data models for vertex_stillness (Production).
...
class ModuleConfig(BaseModel):
    """Configuration model for vertex_stillness"""

    model_config = ConfigDict(frozen=True)

    name: str = Field(default="Vertex Stillness", description="Module name")
    strict_mode: bool = Field(default=True, description="Fail on missing critical settings")
    require_flow_logs: bool = Field(default=True, description="Require VPC flow logs enabled")
    require_nat_gateway: bool = Field(default=True, description="Require NAT gateway for private subnets")
