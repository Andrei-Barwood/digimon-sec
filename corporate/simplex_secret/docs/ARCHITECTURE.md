# Arquitectura - simplex_secret

## Visión General

simplex_secret es un módulo de ciberseguridad implementado como parte del **Snocomm Security Suite**.
...
La clase `SimplexSecret` es el punto de entrada principal. Implementa la lógica de negocio y orquesta las operaciones de seguridad.

**Responsabilidades:**
- `redact_pii()`: Redacta PII de un texto
...
**Modelos Principales:**
- `ModuleConfig`: Configuration model for simplex_secret
- `RedactionResult`: Result of data redaction
...
```python
config = {
    "redaction_mode": "mask",  # Configura redaction_mode
}
```
