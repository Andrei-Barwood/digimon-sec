# Arquitectura - lemniscate_anon

## Visión General

lemniscate_anon es un módulo de ciberseguridad implementado como parte del **Snocomm Security Suite**.

- **Misión**: Charlotte Balfour
- **Rol de Seguridad**: anonymizer
...
La clase `LemniscateAnon` es el punto de entrada principal. Implementa la lógica de negocio y orquesta las operaciones de seguridad.

**Responsabilidades:**
- `anonymize_field()`: Anonimiza un campo individual
- `anonymize_data()`: Anonimiza un diccionario de datos
- `analyze()`: Ejecuta análisis: anonimizar datos
...
**Modelos Principales:**
- `ModuleConfig`: Configuration model for lemniscate_anon
- `AnonymizationRecord`: Record of an anonymization operation
- `AnonymizationResult`: Result of anonymization operation
...
```python
config = {
    "anonymization_method": "pseudonymize",  # Configura anonymization_method
}
```
