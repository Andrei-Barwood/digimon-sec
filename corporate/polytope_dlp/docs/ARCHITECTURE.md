# Arquitectura - polytope_dlp

## Visión General

polytope_dlp es un módulo de ciberseguridad implementado como parte del **Snocomm Security Suite**.

- **Misión**: The New Austin
- **Rol de Seguridad**: data-loss-prevention
...
La clase `PolytopeDlp` es el punto de entrada principal. Implementa la lógica de negocio y orquesta las operaciones de seguridad.

**Responsabilidades:**
- `scan_content()`: Escanea contenido en busca de violaciones de políticas
- `analyze()`: Ejecuta análisis: un contenido o múltiples
...
**Modelos Principales:**
- `ModuleConfig`: Configuration model for polytope_dlp
- `PolicyViolation`: Policy violation information
- `DLPAnalysis`: Result of DLP analysis
...
```python
config = {
    "detection_modes": ["content", "context", "behavior"],  # Configura detection_modes
    "sensitivity_level": "medium",  # Configura sensitivity_level
}
```
